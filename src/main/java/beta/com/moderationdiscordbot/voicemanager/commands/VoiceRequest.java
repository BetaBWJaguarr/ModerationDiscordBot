...
public class VoiceRequest extends ListenerAdapter {
    ...
    private void handleRequest(SlashCommandInteractionEvent event) {
        ...
        if (voiceChannel == null) {
            event.replyEmbeds(embedManager.createEmbed("commands.voiceaction.error.title", "commands.voiceaction.error.not_in_channel", serverSettings.getLanguage(event.getGuild().getId())).build()).setEphemeral(true).queue();
            return;
        }
