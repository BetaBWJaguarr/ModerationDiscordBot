...
public class ChannelBanCommand extends ListenerAdapter {
    ...
    public void onSlashCommandInteraction(SlashCommandInteractionEvent event) {
        if (event.getName().equals("channels")) {
            ...
            if (event.getSubcommandName().equals("ban")) {
                ...
                if (userMatcher.find()) {
                    ...
                    event.getGuild().retrieveMemberById(userToBanId).queue(userToBan -> {
                        ...
                        event.replyEmbeds(embedBuilderManager.createEmbed("commands.channel-ban.user_not_found", null, serverSettings.getLanguage(dcserverid)).build()).setEphemeral(true).queue();
                    });
                } else {
