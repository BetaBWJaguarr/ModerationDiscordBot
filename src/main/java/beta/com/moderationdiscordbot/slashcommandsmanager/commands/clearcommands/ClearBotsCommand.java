...
public class ClearBotsCommand extends ListenerAdapter {
    ...
    public ClearBotsCommand(ServerSettings serverSettings, LanguageManager languageManager, HandleErrors errorManager, RateLimit rateLimit) {
    ...
    public void onSlashCommandInteraction(SlashCommandInteractionEvent event) {
        try {
            ...
            if (!event.getSubcommandName().equals("bots")) return;
            ...
            textChannel.getIterableHistory().takeAsync(amount).thenAcceptAsync(messages -> {
                ...
                messages.stream().filter(message -> message.getAuthor().isBot()).forEach(message -> {
